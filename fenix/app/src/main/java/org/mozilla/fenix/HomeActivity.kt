...
import org.mozilla.fenix.welcome.WelcomeActivity
...
open class HomeActivity : LocaleAwareAppCompatActivity(), NavHostActivity {
    ...
    private val startWelcomeForResult = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            result: ActivityResult ->
        if (result.resultCode == Activity.RESULT_OK) {
            Util.finishWelcomeScreen(applicationContext)
            checkPermission()
        }
    }
    ...
    val PERMISSION_REQUEST_CODE = 0x100
    private val SETTINGS_ACTIVITY_REQUEST_CODE = 0x200
    ...
    override fun onRequestPermissionsResult(
        ...
        isShowedRequestPermission = true
        if (requestCode == PERMISSION_REQUEST_CODE) {
            var isPermissionGranted = true
//            var message = ""
            if (Util.hasQ() && (checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED || checkSelfPermission(
                    Manifest.permission.ACCESS_FINE_LOCATION
                ) != PackageManager.PERMISSION_GRANTED || checkSelfPermission(Manifest.permission.ACCESS_BACKGROUND_LOCATION) != PackageManager.PERMISSION_GRANTED)
            ) {
                isPermissionGranted = false
//                message = getString(R.string.request_permission_setting_android_q_tlb)
            } else if (Util.hasMarshmallow() && (checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED ||
                        checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED)
            ) {
//                message = getString(R.string.text_ask_permission)
                isPermissionGranted = false
            }
